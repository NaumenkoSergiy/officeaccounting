...
var MessageBoard = (function () {
    function MessageBoard(jQuery, options) {
        ...
        this.$messagesWrapperNext = $("<div/>").addClass("messages-wrapper-next").appendTo(this.$el)
        this.$messagesWrapper = $("<div/>").addClass("messages-wrapper").appendTo( this.$messagesWrapperNext);
        ...
        this.$textBox.autosize({
            callback: function (ta) {
                var messagesHeight = _this.options.height - $(ta).outerHeight() - 30;
                ChatJsUtils.setOuterHeight(_this.$messagesWrapper, messagesHeight);
            }
        });

        this.$next_page = $("<div/>").addClass("next-page").attr("page", 1);
        this.$next_page.prependTo(this.$messagesWrapperNext);
        $("<div/>").addClass("next-page-image").appendTo(this.$next_page);
        this.$next_page.click(function(){
            if($('.chat-text-wrapper p').length / 10. - parseInt($('.chat-text-wrapper p').length / 10) < 0.1)
                _this.options.adapter.server.getMessageHistory(_this.options.chatId, _this.options.conversationId, _this.options.otherUserId, function (messages) {
        ...
        this.options.adapter.server.getMessageHistory(this.options.chatId, this.options.conversationId, this.options.otherUserId, function (messages) {
            for (var i = messages.length-1; i >= 0; i--) {
                _this.addMessage(messages[i], null, false, true);
            }
            _this.adjustScroll();
    ...
    MessageBoard.prototype.adjustScroll = function () {
        $('.messages-wrapper-next').scrollTop($('.messages-wrapper-next')[0].scrollHeight);
    };
    ...
    MessageBoard.prototype.sendMessage = function (messageText) {
        ...
        this.options.adapter.server.sendMessage(this.options.chatId, this.options.conversationId, this.options.otherUserId, messageText, clientGuid, function () {
        });
    };
    ...
    MessageBoard.prototype.addMessage = function (message, clientGuid, scroll, paginate) {
        /// <summary>
        ...
        if (message.ClientGuid && $("p[data-val-client-guid='" + message.ClientGuid + "']").length) {
            ...
            var $lastMessage;
            if(paginate)
            $lastMessage = $("div.chat-message:first");
                else
            $lastMessage = $("div.chat-message:last");
            if ($lastMessage.length && $lastMessage.attr("data-val-user-from") == message.UserFromId.toString()) {
                // we can just append text then
                if(paginate)
                    $messageP.prependTo($("div.chat-message:first .chat-text-wrapper"));
                else
                    $messageP.appendTo($("div.chat-message:last .chat-text-wrapper:last-child"));
                ...
                if(paginate)
                    $chatMessage.prependTo($(".messages-wrapper"));
                else
                    $chatMessage.appendTo($(".messages-wrapper"));
                var user_name;
                gon.users.forEach(function(el, ind, arr){
                    if (el.id == message.UserFromId)
                        user_name = el.email;
                })                
                var $timeWrapper = $("<div/>").addClass("chat-time-wrapper");
                var $userName = $("<div/>").addClass("chat-user-name").text(user_name);
                var $gravatarWrapper = $("<div/>").addClass("chat-gravatar-wrapper");
                var $textWrapper = $("<div/>").addClass("chat-text-wrapper");
                var date;
                var d = new Date();
                if (message.CreatedAtDate){
                    date = message.CreatedAtDate.slice(0,19);
                    date = date.replace("T", "\n");
                    $("<p/>").text("| " + date).appendTo($timeWrapper);
                }else{
                    date = d.getFullYear() + "-" + ('0' + d.getMonth()).slice(-2) + "-" + ('0' + d.getDate()).slice(-2) + " " + ('0' + d.getHours()).slice(-2) + ":" + ('0' + d.getMinutes()).slice(-2) + ":" + ('0' + d.getSeconds()).slice(-2);
                    $("<p/>").text("| " + date).appendTo($timeWrapper);
                }

                if(message.UserFromId == gon.current_user.id){
                    $gravatarWrapper.addClass("from-current-user");
                    $textWrapper.addClass("from-current-user");
                }
                $gravatarWrapper.appendTo($chatMessage);
                $userName.appendTo($chatMessage);
                $timeWrapper.appendTo($chatMessage);
                    $textWrapper.appendTo($chatMessage);
                // add text
                if(paginate)
                    $messageP.prependTo($textWrapper);
                else
                ...
                this.options.adapter.server.getUserInfo(message.UserFromId, function (user) {
                    gon.users.forEach(function(el, ind, arr){
                        if(el.id == message.UserFromId){
                            $img.attr("src", 'http://www.gravatar.com/avatar/' + $.md5(el.email));
                        }
                    });
                });
            }
